...
const actionApprovalRulePath = "/api/v1/projects/project-1/action-approvals/rules"
const actionApprovalRuleIDPath = "/api/v1/projects/project-1/action-approvals/rules/rule-1"
...
func TestCreateActionApprovalRule(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "success", statusCode: http.StatusCreated, body: actionApprovalRuleResponse},
		{name: "success ok", statusCode: http.StatusOK, body: actionApprovalRuleResponse},
...
func TestGetActionApprovalRule(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			c := newTestClient(roundTripperFunc(func(r *http.Request) (*http.Response, error) {
				assert.Equal(t, http.MethodGet, r.Method)
				assert.Equal(t, actionApprovalRuleIDPath, r.URL.Path)
				return &http.Response{
					StatusCode: tt.statusCode,
					Header:     http.Header{"Content-Type": []string{"application/json"}},
					Body:       io.NopCloser(strings.NewReader(tt.body)),
			...
			require.NoError(t, err)
			assert.Equal(t, "rule-1", rule.GetId())
			assert.Equal(t, "test rule", rule.GetDescription())
		})
	}
}
...
func TestListActionApprovalRules(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "success", statusCode: http.StatusOK, body: actionApprovalRulesListResponse, wantCount: 1},
		{name: "empty", statusCode: http.StatusOK, body: `{"actionApprovalRules":[],"totalCount":0}`, wantCount: 0},
		{name: "error", statusCode: http.StatusInternalServerError, body: `{"message":"boom"}`, wantErr: true},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			rules, err := c.ListActionApprovalRules(context.Background(), nil)
			if tt.wantErr {
				assert.Error(t, err)
				assert.Nil(t, rules)
				return
