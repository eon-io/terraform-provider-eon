...
const actionApprovalRuleItemPath = "/api/v1/projects/project-1/action-approvals/rules/rule-1"
...
func TestCreateActionApprovalRule(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, externalEonSdkAPI.ACTION_APPROVAL_OPERATION_ADD_RESTORE_ACCOUNT, rule.GetOperation())
		})
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			c := newTestClient(roundTripperFunc(func(r *http.Request) (*http.Response, error) {
				...
				assert.Equal(t, actionApprovalRuleItemPath, r.URL.Path)
				return &http.Response{
					StatusCode: tt.statusCode,
					Header:     http.Header{"Content-Type": []string{"application/json"}},
					Body:       io.NopCloser(strings.NewReader(tt.body)),
