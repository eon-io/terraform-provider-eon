...
resource "eon_backup_vault" "production_us_east" {
...
output "cmk_vault_details" {
  ...
  value = {
    id                  = eon_backup_vault.production_eu_central_cmk.id
    name                = eon_backup_vault.production_eu_central_cmk.name
    region              = eon_backup_vault.production_eu_central_cmk.region
    vault_account_id    = eon_backup_vault.production_eu_central_cmk.vault_account_id
    provider_account_id = eon_backup_vault.production_eu_central_cmk.provider_account_id
    is_managed_by_eon   = eon_backup_vault.production_eu_central_cmk.is_managed_by_eon
  }
}
...
output "multi_region_vault_ids" {
  ...
  value = {
    for region, vault in eon_backup_vault.multi_region :
    region => vault.id
  }
}
